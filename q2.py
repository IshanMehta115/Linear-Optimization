...
A = np.matrix(np.zeros((no_of_nodes, len(edges_id))))

for i in range(no_of_nodes):
    for j in range(no_of_nodes):
        if(1==adj_matrix[i,j]):
            A[i,edges_id[(min(i,j),max(i,j))]] = 1

b1 = np.matrix(np.ones((no_of_nodes,1)))
temp = np.matrix(np.zeros((no_of_nodes,no_of_nodes)))
for i in range(no_of_nodes):
    temp[i,i] = 1
A1 = np.concatenate((A,temp),1)
C1 = np.concatenate((np.matrix(np.ones((len(edges_id),1))) ,np.matrix(np.zeros((no_of_nodes,1))) ))
...
print("Max number of edges = ",z)
print("edges used are ->")
